...
async def get_html(episode_number):
    print(Fore.YELLOW + "Getting HTML for episode {}".format(episode_number), flush=True)

    url = "https://talkpython.com/{}".format(episode_number)

    async with aiohttp.ClientSession() as client:  # open an aiohttp client session
        async with client.get(url) as response:  # the client gets the url async (awaits for the response)
            response.raise_for_status()
            return await response.text()  # the func awaits untill the contents of response being saved/loaded in memory


def get_title(html, episode_number):
    print(Fore.CYAN + "Getting the TITLE for episode {}".format(episode_number), flush=True)
...
async def get_title_range():
    ...
    for i in range(115, 125):
        tasks.append((i, asyncio.create_task(get_html(i))))  # fist start all tasks (generators)
